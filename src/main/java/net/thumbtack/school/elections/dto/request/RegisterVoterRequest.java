...
public class RegisterVoterRequest {
    ...
    public RegisterVoterRequest(String firstName, String lastName, String middleName, String street, int house, int flat, String login, String password) {
    ...
    public void validate() throws ElectionsException {
        ...
        if (this.street == null || this.street.isEmpty()) {
            throw new ElectionsException(ExceptionErrorInfo.EMPTY_VOTER_STREET);
        }
        if (this.house == 0) {
            throw new ElectionsException(ExceptionErrorInfo.EMPTY_VOTER_HOUSE);
        }
        if (this.login == null || this.login.isEmpty()) {
            throw new ElectionsException(ExceptionErrorInfo.EMPTY_VOTER_LOGIN);
        }
        if (this.login.length() < 3) {
            throw new ElectionsException(ExceptionErrorInfo.WRONG_VOTER_LOGIN_LENGTH);
        }
        if (this.password == null || this.password.isEmpty()) {
            throw new ElectionsException(ExceptionErrorInfo.EMPTY_VOTER_PASSWORD);
        }
        if (this.password.length() < 8) {
            throw new ElectionsException(ExceptionErrorInfo.WRONG_VOTER_PASSWORD_LENGTH);
        }
    }
}
